SBC research note images

Place the image files for `research/sbc-demand-classification.html` in this folder.

Expected filenames:

- `linkedin-preview.png` - Chinese social sharing preview (LinkedIn, Slack, etc.)
- `linkedin-preview.en.png` - English social sharing preview, 1200 x 630 recommended
- `01-chart-paradigm.png` - SBC demand classification scatter plot.
- `02-formulas.png` - Optional ADI and CV2 formula illustration.
- `03-quadrants.png` - Four-quadrant classification illustration.

After deployment, social platforms must fetch images over public HTTPS. The HTML pages use absolute URLs under `https://reayhuang.com/`. If you use a custom domain, update `og:url`, `og:image`, and `twitter:image` in both HTML files.
