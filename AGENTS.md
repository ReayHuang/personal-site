# AGENTS.md

## Cursor Cloud specific instructions

This is a **static HTML/CSS/JS personal website** with zero build tools, no package manager, and no dependencies to install.

### Running the site locally

```bash
python3 -m http.server 8080
```

All HTML files are served as-is from the repository root. No build step is required.

### Pages

Bilingual pairs and locale mapping are maintained in `i18n/manifest.json` (`defaultLocale`: `en`).

**Site**

| Path | Description |
|------|-------------|
| `index.html` | English homepage (default) with hero banner |
| `index.zh.html` | Traditional Chinese homepage |
| `index.en.html` | Redirect to `index.html` (legacy URL) |
| `about.en.html` | About / CV (English) |
| `about.zh.html` | About / CV (Traditional Chinese) |
| `notes/index.en.html` | Maritime notes listing (English) |
| `notes/index.zh.html` | Maritime notes listing (Traditional Chinese) |
| `research/index.en.html` | Management notes listing (English) |
| `research/index.zh.html` | Management notes listing (Traditional Chinese) |

**Maritime notes** (`notes/maritime/`) — order matches `notes/index.en.html`

| Slug | English | Traditional Chinese |
|------|---------|---------------------|
| Pilot transfer arrangements | `pilot-transfer-arrangements.en.html` | `pilot-transfer-arrangements.zh.html` |
| oCCS retrofit guide | `occs-retrofit-guide.en.html` | `occs-retrofit-guide.zh.html` |
| Why newbuildings need sea trials | `sea-trial-note.en.html` | `sea-trial-note.zh.html` |
| Can drones replace ship surveyors (RIT) | `can-drones-replace-ship-surveyors.en.html` | `can-drones-replace-ship-surveyors.zh.html` |
| Ammonia fuel | `ammonia-fuel-surveyor-notes.en.html` | `ammonia-fuel-surveyor-notes.zh.html` |
| Future IMO/ILO container ships (2026) | `future-imo-ilo-container-ships-2026.en.html` | `future-imo-ilo-container-ships-2026.zh.html` |
| Container ship Wind Shield | `container-ship-wind-shield-notes.en.html` | `container-ship-wind-shield-notes.zh.html` |
| Electronic certificates (joint guidelines) | `electronic-certificates-joint-guidelines-notes.en.html` | `electronic-certificates-joint-guidelines-notes.zh.html` |
| Onboard certificates | `onboard-certificates.en.html` | `onboard-certificates.zh.html` |
| Engine FAT / shipboard trial (LR-RU-001) | `engine-fat-shipboard-note.en.html` | `engine-fat-shipboard-note.zh.html` |
| LRPF (LR-RU-012) | `lfpf-note.en.html` | `lfpf-note.zh.html` |
| Tailshaft survey | `tailshaft-survey-notes.en.html` | `tailshaft-survey-notes.zh.html` |
| Steering gear | `steering-gear-notes.en.html` | `steering-gear-notes.zh.html` |
| LR BWTS | `lr-bwts-notes.en.html` | `lr-bwts-notes.zh.html` |
| Maritime cyber resilience | `maritime-cyber-resilience-notes.en.html` | `maritime-cyber-resilience-notes.zh.html` |
| LR Advisory | `lr-advisory-notes.en.html` | `lr-advisory-notes.zh.html` |
| MARPOL framework | `marpol-structure-notes.en.html` | `marpol-structure-notes.zh.html` |
| SOLAS framework | `solas-structure-notes.en.html` | `solas-structure-notes.zh.html` |
| Explosion protection | `explosion-protection-notes.en.html` | `explosion-protection-notes.zh.html` |
| LPG fuel | `lpg-fuel-notes.en.html` | `lpg-fuel-notes.zh.html` |
| New construction PM | `new-construction-pm-notes.en.html` | `new-construction-pm-notes.zh.html` |
| Surveyor 6S / lean | `surveyor-6s-lean-management-notes.en.html` | `surveyor-6s-lean-management-notes.zh.html` |
| IMO MSC 111 | `msc-111-visual-notes.en.html` | `msc-111-visual-notes.zh.html` |
| GMDSS | `gmdss-radiocommunications-notes.en.html` | `gmdss-radiocommunications-notes.zh.html` |
| LNG fuel | `lng-fuel-notes.en.html` | `lng-fuel-notes.zh.html` |
| Methanol fuel | `methanol-fuel-notes.en.html` | `methanol-fuel-notes.zh.html` |
| MEPC 83 / 84 | `mepc-83-84-visual-notes.en.html` | `mepc-83-84-visual-notes.zh.html` |

**Korean supplementary pages** (`notes/maritime/`) — standalone; not on listing pages or in `i18n/manifest.json`

| Slug | Korean |
|------|--------|
| SOLAS framework | `solas-structure-notes.ko.html` |
| MARPOL framework | `marpol-structure-notes.ko.html` |
| oCCS retrofit guide | `occs-retrofit-guide.ko.html` |

These pages include `hreflang` links to EN / 繁中 / ko and use `data-back-to-top-label="맨 위로"` on `<html>`.

**Management notes** (`research/`) — order matches `research/index.en.html`

| Slug | English | Traditional Chinese |
|------|---------|---------------------|
| Digital ship management report | `digital-ship-management-report.en.html` | `digital-ship-management-report.zh.html` |
| Owned vs chartered container ships | `owned-vs-chartered-container-ships.en.html` | `owned-vs-chartered-container-ships.zh.html` |
| Fleet spares thesis | `fleet-spares-thesis-notes.en.html` | `fleet-spares-thesis-notes.zh.html` |
| 5S study | `5s-study-notes.en.html` | `5s-study-notes.zh.html` |
| Business statistics | `stats-business-application.en.html` | `stats-business-application.zh.html` |
| SBC demand classification | `sbc-demand-classification.en.html` | `sbc-demand-classification.zh.html` |

Link preview images for listing/homepage cards live in `assets/images/research/` — e.g. `digital-ship-management-linkedin-preview-1200.png` (繁中) and `digital-ship-management-linkedin-preview.en-1200.png` (EN). Hero backgrounds for standalone research articles use optimized `.webp` / `.jpg` pairs (plus `-mobile` variants for narrow viewports), e.g. `hero-digital-ship-management.jpg`, `hero-owned-vs-chartered-fleet-strategy.webp`.

### Internationalization

**Visitor-facing default is English** (`index.html` at site root; `x-default` hreflang points to English). **Authoring starts in Traditional Chinese.**

- **Traditional Chinese (write first)**: new pages must use the `.zh.html` suffix from the start (e.g. `about.zh.html`, `notes/index.zh.html`, `notes/maritime/{slug}.zh.html`). Never create a Chinese page as unmarked `{slug}.html`.
- **English (add after)**: `{slug}.en.html` (root homepage remains `index.html`).
- **Korean (optional)**: `{slug}.ko.html`; not tracked in `i18n/manifest.json` unless added explicitly.
- **Language switch**: EN link appears before 繁中 in the header on every page.
- **Adding content**: copy an existing `*.zh.html` sibling as the template, add the card to the Chinese listing (`notes/index.zh.html` or `research/index.zh.html`), then add `*.en.html` and the English listing when the English version is ready. Update `i18n/manifest.json` with the EN/zh pair.
- **Legacy Chinese URLs**: unmarked `.html` paths (pre-`.zh` rename) 301 to `*.zh.html` via `_redirects`.

See `README.md` for file naming and listing conventions. For machine-readable locale pairs, see `i18n/manifest.json`. Stage handover (2026-09-13 homepage + `.zh.html` rename): `docs/handover-2026-09-13.md`.

### Lint / Test / Build

- **Lint**: No linter configured. HTML can be validated with any W3C HTML validator if needed.
- **Test**: No automated tests. Manual verification by loading pages in a browser.
- **Build**: None required — pure static files.

### Adding content

Copy an existing Traditional Chinese HTML file as a template (`notes/maritime/*.zh.html` or `research/*.zh.html`). The new Chinese page must be named `{slug}.zh.html` from the start. Add a card on the Chinese listing (`notes/index.zh.html` or `research/index.zh.html`), then add `{slug}.en.html` and the English listing when the English version is ready. Add the EN/zh paths to `i18n/manifest.json`. See `README.md` for details.

For recent articles, use `card card--preview` on listing pages and the homepage “Latest content” section, with a `card__preview` `<img>` pointing at the locale-specific LinkedIn preview PNG (see Management notes asset naming above).

### Article page scripts (`</body>`)

Standalone note/article pages under `notes/maritime/` and `research/` also load (after `note-standalone.css` in `<head>`):

1. **`article-share.js`** — LinkedIn / Facebook / copy link / email share buttons (`[data-article-share]`)
2. **`back-to-top.js`** — fixed “back to top” button; label from `<html lang>` or `data-back-to-top-label`

Listing pages (`notes/index.*`, `research/index.*`) and the site root use **`main.js`** (mobile nav toggle) instead.

### Optional maintainer scripts (`scripts/`)

Not required to run the site. Used occasionally to regenerate assets:

| Script | Purpose |
|--------|---------|
| `scripts/generate-homepage-og.py` | Homepage Open Graph preview image |
| `scripts/generate-explosion-protection-og.py` | Explosion protection note OG image |
| `scripts/build-onboard-certificates-en.py` | Regenerate English onboard certificates page content |

### Deployment

Production: **Netlify** at `https://reayhuang.com` (static files, no build step). HSTS enabled. Legacy Traditional Chinese URLs are mapped in `_redirects`.

### Site-wide scripts (`<head>`)

Every HTML page must include the following at the start of `<head>` (copy the three `<script>` lines from a sibling page in the same directory when adding new pages):

1. **Google tag (gtag.js)** — async loader for `G-YJDKH6WYT1`
2. **`gtag-config.js`**
3. **`copy-source-attribution.js`** — immediately after `gtag-config.js`; appends source attribution when users copy selections longer than 100 characters (skips `input`, `textarea`, `[contenteditable]`, `pre`, and `code`)

Relative path to `assets/js/` by page depth:

| Depth | Example path | Script `src` prefix |
|-------|--------------|---------------------|
| 0 | `index.html`, `about.en.html` | `assets/js/` |
| 1 | `notes/index.en.html`, `research/index.zh.html` | `../assets/js/` |
| 2 | `notes/maritime/*.en.html` | `../../assets/js/` |

To verify all pages include the copy script: `grep -L "copy-source-attribution" **/*.html` (no output means every file is covered).
